...
constexpr int NUMBER_OF_OPPONENTS = 3;

void initialize_players(entt::registry &registry) {
    const auto player = registry.create();
    registry.emplace<Player>(player, "Player", 0); // add player component

    for (int i = 0; i < NUMBER_OF_OPPONENTS; i++) {
        const auto opponent = registry.create();
        registry.emplace<Player>(opponent, "Opponent " + std::to_string(i + 1), 0); // add opponent component
    }
}

void initialize_deck(entt::registry &registry) {
    const auto deck = registry.create();
    registry.emplace<Deck>(deck); // add deck component
    registry.get<Deck>(deck).initialize(); // initialize deck
}

void list_of_cards(entt::registry &registry) {
    const auto deck = registry.view<Deck>();
    for (const auto entity: deck) {
        const auto &deck = registry.get<Deck>(entity).getCards();
        for (const auto &card: deck) {
...
void deal_cards(entt::registry &registry, const int numberOfCards) {
    auto deckEntity = registry.view<Deck>().front();
    auto &deck = registry.get<Deck>(deckEntity);

    auto playerEntities = registry.view<Player>();

    for(auto playerEntity: playerEntities) {
        auto &player = registry.get<Player>(playerEntity);
        player.hand.clear();

        if (deck.getCards().size() >= numberOfCards * (NUMBER_OF_OPPONENTS + 1)) {
            for (int i = 0; i < numberOfCards; i++) {
                // Draw a card from the deck and add it to the player's hand
                player.hand.push_back(deck.getCards().back());
                deck.getCards().pop_back();
                std::cout << player.getName() << " has " << player.hand[i].get()->getName() << std::endl;
            }
        } else {
            std::cout << "Not enough cards in the deck!" << std::endl;
        }
...
int main() {
    ...
    deal_cards(registry, 2);

    // deal_cards(registry, 2);

    // list_of_cards(registry);
